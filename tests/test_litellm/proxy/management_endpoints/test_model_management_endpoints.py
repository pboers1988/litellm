...
class TestTeamModelUpdate:
    ...
    async def test_a_refused_row_write_leaves_the_team_untouched(self):
        ...
        patch_data = updateDeployment(model_name="team-public", model_info=ModelInfo(team_id="team_123"))
    ...
    @pytest.mark.asyncio
    async def test_rename_with_prisma_none_keeps_the_internal_model_name(self):
        """A team row keeps its internal UUID-based model_name: the user-supplied public name goes to
        model_info, and the row write must never receive it as model_name, prisma or no prisma."""
        from litellm.proxy.management_endpoints.model_management_endpoints import (
            _update_team_model_in_db,
        )
        from litellm.types.router import ModelInfo
        ...
        db_model = Deployment(
            model_name="model_name_team_123_abc",
            litellm_params=LiteLLM_Params(model="azure/gpt-4o-mini"),
        ...
        async def write_row(update_data):
        ...
        assert written["model_name"] == "model_name_team_123_abc"
        assert "new-public-name" in str(written["model_info"])
...
class TestStrategyRouterWriteValidation:
    ...
    async def test_team_model_bookkeeping_runs_after_the_slot_is_released(self) -> None:
        ...
        async def slot():
            ...
            yield MagicMock(litellm_proxymodeltable=MagicMock(create=AsyncMock(return_value=created)))
