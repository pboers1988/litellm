...
def count_heuristic_v2_routers(
    deployments: Iterable[Mapping[str, object]], *, exclude_model_id: str | None = None
) -> int:
    """How many of ``deployments`` (router model_list entries or config.yaml rows) are heuristic_v2 routers.

    ``exclude_model_id`` leaves out the deployment being edited, so it keeps its own slot.
    """
    return sum(
        1
        for deployment in deployments
        if (exclude_model_id is None or _mapping(deployment.get("model_info")).get("id") != exclude_model_id)
        and is_heuristic_v2_router(_mapping(deployment.get("litellm_params")))
    )
