...
async def _heuristic_v2_slot(
...
) -> AsyncGenerator[_TxModelTables, None]:
    """Run a model write while its claim on a heuristic_v2 slot is settled in the database.
    ...
    an unlimited license, runs on the plain client with no lock. Only the row write itself may
    run inside: anything that needs a second connection (the team model bookkeeping) must wait
    until the transaction has committed and the lock is released.
    """
    ...
    try:
        ...
        async def write_row(update_data: PrismaCompatibleUpdateDBModel) -> _ProxyModelRow | None:
            ...
            async with _heuristic_v2_slot(
                ...
                return await tables.litellm_proxymodeltable.update(where={"model_id": model_id}, data=update_data)
...
async def _add_model_to_db(
    ...
    slot: AbstractAsyncContextManager[_TxModelTables] | None = None,
    ...
    async with slot as tables:
        return await tables.litellm_proxymodeltable.create(data=_create_data)
    ...
    try:
        ...
        if store_model_in_db is True:
            ...
            async with _heuristic_v2_slot(
                ...
                model_response: Final = await tables.litellm_proxymodeltable.update(
